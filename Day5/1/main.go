...
func main() {
	...
	locations := findBestSpots(seeds, seedMap, fromToMap)
	sort.Ints(locations)
	fmt.Println("Best location: ", locations[0])
}
...
func findBestSpots(seeds []int, seedMap map[string]mapping, fromToMap map[string]fromTo) []int {
	...
	for _, seed := range seeds {
		...
		locations = append(locations, findBestSpotForSeed(seed, seedMap, fromToMap))
		fmt.Println("- Location: ", locations[len(locations)-1])
	}
	return locations
}
